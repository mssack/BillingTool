...
#pragma warning disable 1591
	public class TimeEditor : EditorBase<DateTime?>
	{
		#region DependencyProperty Static Keys
		public static readonly DependencyProperty AllowNullProperty = DependencyProperty.Register("AllowNull", typeof (bool), typeof (TimeEditor), new FrameworkPropertyMetadata {DefaultValue = default(bool), DefaultUpdateSourceTrigger = UpdateSourceTrigger.PropertyChanged, PropertyChangedCallback = (o, args) => ((TimeEditor) o).AllowNullChanged((bool) args.OldValue, (bool) args.NewValue)});
		internal static readonly DependencyProperty InternalValueProperty = DependencyProperty.Register("InternalValue", typeof (DateTime?), typeof (TimeEditor), new FrameworkPropertyMetadata {DefaultValue = default(DateTime?), BindsTwoWayByDefault = true, DefaultUpdateSourceTrigger = UpdateSourceTrigger.PropertyChanged, PropertyChangedCallback = (o, args) => ((TimeEditor) o).InternalValueChanged((DateTime?) args.OldValue, (DateTime?) args.NewValue)});
		#endregion
		...
		private void InternalValueChanged(DateTime? oldValue, DateTime? newValue)
		{
			...
			using (_changeLock.Activate())
			{
				...
				if (Value == null)
				{
					Value = new DateTime(1, 1, 1, newValue.Value.Hour, newValue.Value.Minute, newValue.Value.Second, 0);
					return;
				}
				Value = new DateTime(Value.Value.Year, Value.Value.Month, Value.Value.Day, newValue.Value.Hour, newValue.Value.Minute, newValue.Value.Second, Value.Value.Millisecond);
			}
		}
